...
<header>
  ...
  <!-- Dar estilo al boton Nuevo registro, con su respectivo link-->
<br>


<div class="container m-auto size-11/12">
    <button class="bg-green-500 hover:bg-green-700 text-white font-bold
    py-1 px-2 border border-green-500 rounded"><a href="{{
        ...
        <tr class="bg-gray-300 border border-grey-500 md:border-none block md:table-row">
            ...
            <td class="p-2 md:border md:border-grey-500 text-left block md:table-cell">
                ...
                <form action="{{route('producto.borrar', $produ)}}" method="POST">
                    ...
                    @if($produ->deleted_at)
                        <button type="button" class ="bg-green-500 hover:bg-green-700 text-white font-bold py-1 py-2 border border-green-500 rounded"><a href="{{route('activapro',$produ->id)}}">activar</a></button>
                        <input class ="bg-red-500 hover:bg-red-700 text-white font-bold py-1 py-2 border border-red-500 rounded" type="submit" value="borrar" onclick="if(!comfirm('desea eliminar a:{{$produ->nombre}}?')){return false;}"/>

                
                @else

                    <button type="button" class ="bg-red-500 hover:bg-red-700 text-white font-bold py-1 py-2 border border-red-500 rounded"><a href="{{route('desactivapro',$produ->id)}}">desactivar</a></button>
